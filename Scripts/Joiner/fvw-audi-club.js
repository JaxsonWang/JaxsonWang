...
function getToken() {
  ...
  const _openid = body['user_bury'].openid
  const _aid = typeof body['user_bury'].aid === 'number' ? String(body['user_bury'].aid) : body['user_bury'].aid
  if (_openid) $.setdata(_openid, openidKey)
  if (_aid) $.setdata(_aid, aidKey)
}
...
function getHeaders() {
  return {
    Cookie: token,
    'User-Agent':
      'Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.49(0x18003135) NetType/WIFI Language/zh_CN'
  }
}
...
async function getSignIn1Request() {
  const options = {
    ...
    headers: getHeaders(),
    body: `aid=${Number(aid)}&openid=${openid}`
  }
...
async function getSignIn2Request() {
  const options = {
    ...
    headers: getHeaders(),
    body: `url=%2FshoppingIndex.html&aId=${Number(aid)}`
  }
  return new Promise(resolve => {
    $.post(options, async (error, response, data) => {
      const result = JSON.parse(data)
  ...
  return new (class {
    ...
    debug(...logs) {
      if (this.logLevels[this.logLevel] <= this.logLevels.debug) {
        ...
        console.log(`${this.logLevelPrefixs.debug}${logs.map(l => l ?? String(l)).join(this.logSeparator)}`)
      }
    }
    ...
    info(...logs) {
      if (this.logLevels[this.logLevel] <= this.logLevels.info) {
        ...
        console.log(`${this.logLevelPrefixs.info}${logs.map(l => l ?? String(l)).join(this.logSeparator)}`)
      }
    }
    ...
    warn(...logs) {
      if (this.logLevels[this.logLevel] <= this.logLevels.warn) {
        ...
        console.log(`${this.logLevelPrefixs.warn}${logs.map(l => l ?? String(l)).join(this.logSeparator)}`)
      }
    }
    ...
    error(...logs) {
      if (this.logLevels[this.logLevel] <= this.logLevels.error) {
        ...
        console.log(`${this.logLevelPrefixs.error}${logs.map(l => l ?? String(l)).join(this.logSeparator)}`)
      }
    }
    ...
    logErr(err, msg) {
      switch (this.getEnv()) {
        ...
        default:
          ...
          break
        case 'Node.js':
          this.log('', `❗️${this.name}, 错误!`, msg, typeof err.message !== 'undefined' ? err.message : err, err.stack)
          break
      }
    }
